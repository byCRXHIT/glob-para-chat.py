...
async def addGlobal(ctx):
    if ctx.author.guild_permissions.administrator:
        if not guild_exists(ctx.guild.id):
            ...
            with open('servers.json', 'w') as f:
                json.dump(servers, f, indent=4)
            embed = discord.Embed(title="**Welcome in the GlobalChat from the Paradise™ server**",
                                  ...
                                  color=0x2ecc71)
            embed.set_footer(text='Please keep in mind that this server need an 5 sec '
                                  ' slowmode because of spamming!')
...
async def sendAll(message: Message):
    ...
    links = '[Check out FakeCT!](https://discord.gg/j6nAhV6) ║ '
    ...
    embed.add_field(name='Extra', value=links, inline=False)
